...
## 2026-09-03：4,173文件库存、2,024份官方材料与分层EvidencePack

本地`laws`目录的4,173个文件是混合库存，不是4,173条训练样本或高质量法源。当前实盘为：

- 1,458 DOCX、53 DOC、2,616 TXT、30 ZIP及脚本/缓存等；“530”是案例文本数量，不是DOC数量；
- 1,511个原始来源文档、2,024个canonical派生文本、61个汇总文本、30个归档、545个运维文件和2个缓存；
- 2,024份canonical法律、行政法规、司法文件和案例均已对应国家法律法规库等官方来源原件，并各自生成来源与效力元数据记录；官方原件默认准入，不再要求逐份联网或模型复核；
- 当前效力状态为`verified_current` 1,045、`verified_historical` 598、`superseded` 5、`repealed` 13、`unresolved` 363；`unresolved`不阻塞使用，但必须提示效力尚未完全核实；
- 刑法505条与刑诉法308条合计813条，是当前刑法课程核心规范基线，不是多来源EvidencePack的唯一可引用范围；
- 法律/行政法规用于规范依据，司法解释/司法规范性文件用于司法适用，案例用于裁判参考，教材用于课堂解释，公开题用于学习资源；低层级材料不得覆盖高层级规范。
...
- `governed_source_manifest.json`：813条刑法课程核心规范基线、课程层及历史准入策略；
- `OFFICIAL_SOURCE_VERIFICATION_V1.jsonl`：2,024份官方材料逐份来源、元数据、效力与使用状态；
- `OFFICIAL_SOURCE_VERIFICATION_V1_SUMMARY.json`和`OFFICIAL_SOURCE_VERIFICATION_V1_REPORT.md`：聚合统计、非阻断未解项与使用边界；
...
初始库存构建过程模型调用0、源文件修改0；已有流水线完成确定性原件/元数据匹配，并保留此前官方抽检和模型核实结果。四类材料均来自国家法律法规库等官方来源，不再为缺失详情页或效力字段逐份深挖；无法完全确定的效力状态保留为非阻断提示，只在明确版本冲突、废止线索或引用错配时异常复核。正式课堂前仍应按届时版本增量刷新现行法状态。
...
- 含“中国刑事辩护网提供”署名且存在逐条内容差异的第三方“2024最新版”参考件已隔离，不参与构建；隔离针对具体文件污染/一致性问题，不否定2024-03-01生效的现行刑法版本；
