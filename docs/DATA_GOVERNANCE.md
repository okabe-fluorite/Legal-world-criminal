...
- 2,024份canonical法律、行政法规、司法文件和案例均已对应国家法律法规库等官方来源原件，并各自生成核实记录；
...
初始库存构建过程模型调用0、源文件修改0；后续核实流水线先做确定性原件/元数据匹配，再合并并行模型核实结果。司法文件均来自国家法律法规库下载原件，不再为缺失详情页逐份阻塞；无法完全确定的效力状态保留为非阻断提示。正式课堂前仍应按届时版本增量刷新现行法状态。
