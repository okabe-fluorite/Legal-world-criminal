...
可信RAG、AI分层解惑和实时语音回复共享`EvidenceCitations.vue`组件。后端仍用稳定Evidence ID关联资料，但学生界面不展示内部ID、快照或SHA；悬停显示截断摘要，点击抽屉只展示来源类型、法源层级、条号、引用原文、版本/时效、友好使用提示和原始链接。引用检查只验证来源存在、范围和逐字片段，不自动证明法律蕴含；抽屉和回复均保留教师/专家复核提示。
