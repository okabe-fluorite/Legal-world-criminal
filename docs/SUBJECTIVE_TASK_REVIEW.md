...
学生重新打开主观训练时，“我的复核台账”会显示任课教师的批准、退回或拒绝结论，但不显示教师账号或用户ID。退回稿可一键带入原文，以新`attempt_id`提交修订；旧稿和旧决定保持不可变。批准稿会显示教师形成性分、知识状态和对应Evidence事件ID。
...
| `GET /api/subjective-attempts?phase=...` | 当前学生自己的稿件及去标识化教师反馈历史 |
...
- 同一稿件只能接受一次教师决定；同review ID同payload幂等，换review ID再审冲突；
- 教师退回→学生带入原文修订→新稿再审→教师批准→学生看到Evidence的真实浏览器闭环通过。
...
- **视频第5段（约20秒）**：教师退回→学生带入修订→教师批准→画像事件增加；
- **PPT第8页**：Human-in-the-loop与AI形成性反馈边界；
- **赛题评分项**：创新应用、垂类模型可信性、教学闭环、数据与内容安全；
- **仍需真实证据**：至少2名目标用户试用；当前浏览器旅程和模型样本只能证明功能可运行，不能替代真实学生/教师体验评价。
