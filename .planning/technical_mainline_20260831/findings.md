...
## 阶段13产品定位与视觉结论（2026-09-01）
- “技术证据”页的唯一合理定位是比赛视频、PPT答辩和队友验收的只读技术总账：集中投影数据治理、推理Gate、LegalEduEval、Agent消融、哈希和pending边界。它不是学生学习任务，也不应进入作答、评分、LearningEvent或画像。
- 课前/课后共用TaskAttempt契约是正确的安全/审计复用，但此前学生界面只换阶段标签，视觉和任务心智过于相似。阶段13把课前定义为“问题先行的入课准备”，把课后定义为“证据驱动的错因复盘”，同时改变流程、栏目、按钮、困惑提示和配色。
- 1500×980最终门禁测得自主学习题干23.25px、选项15.68px；780×900题干19.2px、选项15.68px。课前/课后强调色分别为暖色`rgba(176,138,62,.42)`和冷色`rgba(100,156,168,.48)`，切换后3个滚动栏均归零。
- 技术证据页桌面/窄屏最小受测字号为标签12.48px、pipeline说明10.88px、边界10.4px；四页横向溢出0、私有字段0、console/page/HTTP/request错误0。
