...
### 阶段13：学习阶段差异化与录屏可读性
- [x] README和演示文档删除本机绝对`.env.example`路径，改为通用外部配置占位与本地`.env`直接启动
- [x] 明确“技术证据”为比赛/答辩/队友验收只读总账，不参与学生作答、评分、LearningEvent或长期画像
- [x] 课前预习改为暖色“入课问题单”，流程为浏览规则→完成摸底→标记困惑→带入课堂
- [x] 课后复习改为冷色“证据复盘台”，流程为回看错因→核对Evidence→完成变式→安排再测
- [x] 放大自主学习与技术证据关键字号，并验证1500×980和780×900无溢出/错误
- [x] 更新公开证据、文档、提交并推送
- **状态：** completed_machine_scope（真实用户可理解性与学习增益仍需试点）
...
| 阶段13首次自主学习冒烟仍等待旧标题“知识卷宗” | 1 | 页面差异化后标题改为“本课知识地图/错因与知识清单”；同步smoke等待新课前标题后重跑 |
| 阶段13第二次自主学习冒烟仍匹配旧困惑占位提示 | 1 | 页面按阶段使用不同语义占位；smoke改为正则匹配课前/课后两种提示 |
| 阶段13首次提交消息中的中文双引号被PowerShell截断 | 1 | 提交未发生、暂存内容未变；改用here-string变量传给`git commit -m` |
| 阶段13完成后教师冒烟仍查找旧主观入口文案 | 1 | 页面功能正常；测试改为兼容课前“写下初步论证”和课后“重写/角色互换”入口 |
| 教师冒烟在未配置teacher白名单的普通本地栈中注册教师账号后仍为student | 1 | 这是测试启动前置条件，不修改权限；用唯一`SIMLAW_TEACHER_EMAILS/TEACHER_SMOKE_EMAIL`重启测试栈后重跑 |
