...
- 项目级缺口审计发现后端多来源EvidencePack此前没有学生自由输入的通用前端入口；已在可信RAG内补齐，不新增顶栏。1500×980与780×900各用5个真实查询验证法律、司法文件、案例父段、教材、公开题和效力提示，并验证不存在法名/条号的可见引用为0。
- 个性化路径审计发现旧推荐器虽有固定章节顺序，却没有真正用`prerequisite_ids`约束下一题，且学习卷宗按当前知识点过滤队列会绕过跨知识点推荐。现升级为递归先修前沿：客观薄弱先选最根部未满足先修，完整路径回传；节点只有`mastered+provisional(3事件/2题)`才解锁下一层。困惑自报仍直接回应当前知识，不作负掌握。
- 代码完成审计经无上下文读者测试后，改为“本地代码实现/外部验收提交”双列，并补足六种来源身份与五种用途组、57,051检索记录口径、单案例/单轮语音边界、三路线名称、外部负责人和最终提交清单，避免把代码完成误读为比赛或教学效果完成。
- 用户再次确认法律/法规/司法解释/案例均来自国家法律法规库，不希望继续执着逐份核实。必要抽检结果：335份法律中134份官方详情API直接返回状态码，与Luna前轮状态冲突0；201份限流保留前轮状态并停止重试。国家库当前前端枚举为1已废止、2已修改、3有效、4尚未生效，因此不能凭旧时间记忆推翻2026-09-04官方状态。
- 运行时审计发现精确查询其他法律仍会追加刑法核心BM25，导致目标法在行末引用中被挤出。现增加精确法名保护和不存在法名弃权；明确引用其他法名时不再注入刑法核心命中。已废止材料详情显示“已废止，不得作为现行法依据”。
- 用户最终确认四类材料均来自国家法律法规库等官方来源，要求停止对来源与效力逐份深挖。后续口径为“官方原件默认准入 + 逐份来源记录 + 异常复核”：363项效力字段待完善仅作非阻断提示，不再触发Luna或官方网页批量核验；只对明确版本冲突、废止线索、引用错配做处理。
