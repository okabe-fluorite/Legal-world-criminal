...
## 目标
把2,024份国家法律法规数据库等官方来源canonical法律/法规/司法文件/案例建立逐份来源记录，并按真实身份接入多来源EvidencePack；官方原件默认可用，不再把逐份联网或模型核验作为研发主线。813条保持刑法课程核心规范基线，57,051条保持检索记录口径。沿用既有Embedding，不重复计算向量；完善必要元数据、层级准入、案例父段回填、前端引用和比赛材料。
...
- [ ] Evidence/NLI/端到端RAG
- [ ] 题目答案隔离和教材分层
- [ ] PPT/视频/技术报告（项目功能整体完成后一次性同步；当前只维护开发文档）
- **状态：** pending

### 阶段8：多来源EvidencePack与轻量来源治理
- [x] 扩展Evidence Schema：法律、行政法规、司法文件、案例、教材、学习资源
- [x] 为2,024份官方原件逐份生成来源与效力元数据记录，保留原件对应和增量缓存
- [x] 合并已有确定性元数据、官方状态抽检与历史模型核实结果；停止继续逐份追网页或模型深挖
- [x] 更新运行时层级准入、EvidencePack投影、案例父段回填和无答案弃权
- [x] 更新行末引用悬浮/详情，友好显示身份、效力、父段和使用范围
- [x] 完成8类真实HTTP/降级验证，重跑R0—R4候选消融
- [x] 融合课程知识图与Hybrid RAG：知识节点/先修/条号扩展查询，Evidence反向支撑解释和路径
- [x] 在可信RAG前端提供自由检索，五类来源正文行末引用、悬停、详情、父段、效力提示和无答案弃权可真实操作
- [x] 把KnowledgeCard先修图落入Recommendation：递归先修前沿、3事件/2题逐层解锁、全局队列和路径原因可真实执行
- [x] 完成官方原件信任口径下的必要运行时保护：法律状态码抽检、多轮核实顺序合并、历史/被替代/废止用途提示、精确法名保护
- [x] 同步README与技术报告，统一为“官方原件默认准入、逐份来源记录、异常复核”口径
- [ ] 项目功能整体完成后，一次性同步效果报告、PPT、视频计划和公开DRAFT包
- [ ] 视频V3继续等待用户另行确认，不生成或覆盖
- **状态：** in_progress

### 阶段9：项目代码收口与完成审计
- [x] 完成多来源自由检索、Graph×RAG和递归先修路径的实际用户闭环
- [x] 后端164项、adaptive 13项、产品机制12项与集成验证通过
- [x] 三条核心浏览器路线重跑3/3，错误0且端口无残留
- [x] 区分代码完成、队友模型、教师/用户/签署和数字人后置边界
- [x] 完成最终Git/密钥/追踪文件审查：选择性暂存35项，无密钥/私密文件/绝对本机路径，未触碰PPT/视频遗留修改
- [ ] 代码确认收口后一次性同步PPT、视频计划、效果报告和公开DRAFT包
- **状态：** in_progress
...
- 知识图谱采用课程层轻量DAG，而非把57,051条全文复制为巨型图：KnowledgeCard、先修关系和法条锚点负责诊断/路径/查询扩展，Hybrid RAG负责多来源证据召回和引用，两者双向联动。
- 2026-09-03用户调整交付顺序：在后端、前端、数据、检索、知识图谱、认知诊断和真实运行验证整体完成前，不再频繁修改PPT或视频；当前只更新开发文档，最终集中同步比赛材料。
- 2026-09-04用户最终确认：法律、行政法规、司法解释和指导性案例均来自国家法律法规库等官方来源，来源可信默认成立；2,024条记录用于身份、层级、版本和必要风险提示，不代表每份都要继续联网或调用模型复核。后续仅对明确版本冲突、废止线索或引用错配做异常处理，363项效力字段待完善不驱动大规模复核。
...
| 直接运行uvicorn未补默认DATABASE_URL，浏览器注册返回500 | 1 | 停止该进程，改用仓库`start.py`本地主线启动，由启动器补SQLite配置后重跑，不使用Docker |
| 效果报告表格换行导致精确文本门禁误报缺词 | 1 | 保持事实短语不变，对易被PDF拆行的Hybrid/核心基线/Recall改用去空白检查，并恢复OCR/数字人完整边界 |
| 实时语音smoke仍寻找旧文案“打开原始来源” | 1 | 产品已统一为“打开官方来源”；只同步测试选择器，不改产品行为或放宽URL检查 |
| 图谱路径浏览器测试发现全选选项会因命中正确项被记为partial | 1 | `exact_option_set`改为只有“所选集合是正确集合真子集”才partial；混入任一错误项记missing，并加回归测试 |
| adaptive专项从服务目录运行未设置PYTHONPATH导致模块导入失败 | 1 | 按服务布局设置`PYTHONPATH=src`后重跑13项成功，不改业务导入路径 |
| 三路线彩排0/3且只执行第一条 | 1 | 浏览器错误实际为0；总编排器仍断言旧媒体证明行数、旧机器状态文案和旧RAG结果键，更新为当前公开契约后重跑，不改产品迎合旧测试 |
| 将`verify_criminal.py --help`误当帮助命令实际执行并遇GBK符号错误 | 1 | 脚本无CLI帮助；改用`python -X utf8 backend/scripts/verify_criminal.py`后全部集成验证通过 |
| 国家库法律状态首轮24并发仅75/335直连成功 | 1 | 降至2并发并复用缓存，最终134份成功、201份限流不再重试；与前轮状态冲突0，按用户要求停止深挖 |
| adaptive专项从`adaptive_service`目录直接discover时包不可见 | 1 | 按README从仓库根设置`PYTHONPATH=adaptive_service/src`后重跑，不重复错误入口 |
| 已废止法精确查询被通用刑法BM25候选挤出前端前4引用 | 1 | 增加精确法名保护；明确查询其他法时不再注入刑法核心BM25；不存在法名无条号也可靠弃权 |
