...
async function load(): Promise<void> {
  ...
  error.value = "";
  try {
    const result = await api.subjectiveCatalog(props.phase);
    tasks.value = result.tasks;
    selectTask(result.tasks[0] ?? null);
  } catch (reason) {
    ...
    <section class="subjective-board" role="dialog" aria-modal="true" aria-label="刑法主观论证训练">
      <header class="subjective-head">
        ...
        <div class="subjective-summary mono">
          <span>10 知识短答</span><span>3 角色互换</span><span>教师批准后才入画像</span>
        </div>
...
@media(max-width:900px){.subjective-layer{padding:0}.subjective-summary{display:none}.subjective-body{display:block;overflow-y:auto}.subjective-index,.draft-main{min-height:auto;overflow:visible;border-right:0}.subjective-list{grid-template-columns:repeat(2,minmax(0,1fr))}.draft-grid{display:block}.draft-evidence{margin-top:12px}.feedback-columns{display:block}.feedback-columns section{margin-bottom:8px}.draft-paper footer{align-items:flex-start;flex-wrap:wrap}.char-count{margin-left:0}}
</style>
