...
const pipeline = computed(() => {
  ...
  if (!row) return [];
  return [
    { no: "01", label: "混合文件", metric: row.summary.candidate_files.toLocaleString(), note: "原件、派生、归档与缓存" },
    { no: "02", label: "官方材料", metric: row.summary.canonical_official_documents.toLocaleString(), note: "去重后按真实身份入包" },
    { no: "03", label: "检索记录", metric: row.summary.hybrid_records.toLocaleString(), note: "法源·教材·公开题分层" },
    { no: "04", label: "课程核心基线", metric: row.summary.formal_articles.toLocaleString(), note: "《刑法》505 + 《刑诉法》308" },
    { no: "05", label: "推理检查", metric: `${row.summary.reasoning_gate_checks}项`, note: "结构、来源与引用检查" },
    { no: "06", label: "统一评测", metric: `${row.summary.benchmark_items}题`, note: "候选题 · 待教师确认" },
  ];
});
  ...
  <div class="tech-layer">
    <section class="tech-board" role="dialog" aria-modal="true" aria-label="学科技术说明">
      <header class="tech-head">
        <div class="tech-brand"><span class="tech-seal">证</span><div><p class="kicker mono">TECHNICAL EVIDENCE · TRACEABLE · CLEAR BOUNDARY</p><h2>刑法学科技术说明</h2></div></div>
      ...
      <main v-else class="tech-content">
        ...
        <template v-if="tab === 'overview'">
          <section class="overview-statement"><div><p class="kicker mono">核心技术结果</p><h3>不是“功能很多”，<br>而是数据、证据与学习相互支撑</h3></div><div class="overview-number"><b>{{ evidence.summary.canonical_official_documents.toLocaleString() }}</b><span>去重后的官方材料</span><small>{{ evidence.data_governance.local_source_matched.toLocaleString() }}/{{ evidence.summary.canonical_official_documents.toLocaleString() }}原件对应</small></div></section>
        ...
        <template v-else-if="tab === 'data'">
          <section class="section-title"><div><p class="kicker mono">4,173 → 2,024 → 57,051</p><h3>文件、官方材料与检索记录分层</h3></div><span class="status-pass">2,024份逐份记录已生成</span></section>
          <div class="data-grid"><section class="data-ledger"><article><b>{{ evidence.data_governance.inventory_files.toLocaleString() }}</b><div><h4>混合文件库存</h4><p>原件、派生文本、压缩包、脚本和缓存</p></div><span>FILES</span></article><article><b>{{ evidence.data_governance.canonical_official_documents.toLocaleString() }}</b><div><h4>去重后的官方材料</h4><p>{{ evidence.data_governance.local_source_matched.toLocaleString() }}/{{ evidence.data_governance.canonical_official_documents.toLocaleString() }}对应国家官方来源原件</p></div><span>SOURCE</span></article><article><b>{{ evidence.hybrid_rag.records.toLocaleString() }}</b><div><h4>分层混合检索记录</h4><p>法源案例 {{ evidence.hybrid_rag.collections.legal_authority?.toLocaleString() }} · 教材 {{ evidence.hybrid_rag.collections.textbook_explanation?.toLocaleString() }} · 公开题 {{ evidence.hybrid_rag.collections.question_public?.toLocaleString() }}</p></div><span>RAG</span></article><article><b>{{ evidence.data_governance.verified_current.toLocaleString() }} / {{ evidence.data_governance.unresolved.toLocaleString() }}</b><div><h4>当前有效 / 效力待完善</h4><p>未完全核实项可检索，但引用时必须显式提示</p></div><span>STATUS</span></article><article><b>{{ evidence.data_governance.formal_articles }}</b><div><h4>刑法课程核心规范基线</h4><p>《刑法》{{ evidence.data_governance.criminal_law_articles }}个条文 + 《刑诉法》{{ evidence.data_governance.criminal_procedure_articles }}个条文</p></div><span>CORE</span></article></section>
            <section class="version-audit"><header><p class="kicker mono">2024 CRIMINAL LAW VERSION</p><h3>现行版本锚定至修正案十二施行日</h3></header><div class="version-hero"><b>{{ evidence.data_governance.version_as_of }}</b><span>官方2020正文 + 修正案十二确定性合并</span></div><dl><div><dt>七处修正案匹配</dt><dd>{{ evidence.data_governance.amendment_12_matches }}/7</dd></div><div><dt>参考件清理后逐字一致</dt><dd>{{ evidence.data_governance.reference_exact_articles }}/{{ evidence.data_governance.criminal_law_articles }}</dd></div><div><dt>参考件剩余差异条文</dt><dd>{{ evidence.data_governance.reference_differences }}</dd></div><div><dt>差异参考件参与生产</dt><dd class="reject">{{ evidence.data_governance.reference_formal_admission ? "参与" : "不参与" }}</dd></div></dl><footer>2024版本口径正确；生产正文由官方版本链合并，差异参考件只作交叉校验。</footer></section></div>
        </template>
        ...
        <template v-else>
          <section class="section-title"><div><p class="kicker mono">AGENT C0 / C1 · SAME CASE · SAME MODEL</p><h3>增加反方的收益，必须与成本一起展示</h3></div><span class="status-pending">教师盲评待完成</span></section>
