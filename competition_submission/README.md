...
课前/课后已完成差异化视觉：课前为“带着问题进入课堂”的暖色入课问题单，课后为“用证据完成一次复盘”的冷色证据台；同一账号的桌面截图和字号/配色/滚动回归见`03-Demo/learning-phases/`。“技术说明”页明确标注为比赛/答辩只读视图，不参与学生学习或画像，并已隐藏SHA、内部ID和文件审计细节。
...
- 行末引用抽屉向用户展示来源、版本/时效和原文，不自动证明法律蕴含；内部SHA只保留在技术报告，ASR结果仍需复核且不进入正式评分或长期画像。
