...
- 叙事：问题 → 技术架构 → 数据治理 → 模型接口 → Evidence推理 → LegalEduEval → Agent消融 → 真实应用 → 个性化 → 讯飞ASR/TTS实证 → 边界。
...
- PowerPoint实际打开并导出12页PNG；PPTX 1,644,330字节，16:9，低于100MB。
- PowerPoint回渲染与浏览器验收图最大平均绝对像素差0.094643。
- 第11页经过“截图过小/标题遮挡→横向能力卡证据带→移除叠字”两轮修复与独立复审；相似度1.0显著标注为单条合成验收句，不称ASR准确率。
