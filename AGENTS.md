...
- 顶部“技术证据”打开`TechnicalEvidenceShowcase.vue`，以总账/数据治理/推理评测/Agent边界四页展示已提交机器证据。
- `/api/competition/technical-evidence`每次读取6份权威JSON/manifest并重算SHA，只投影计数、状态、成本、哈希与边界；缺失/损坏时503。
- 响应禁止包含完整Agent推理、A/B内部映射、教师私有字段、题目答案、学生原文、密钥、鉴权头和绝对路径。
- 页面在1500×980与780×900验证：4173/813、11项Gate、100题、C0/C1成本和4项pending均可见，横向溢出与私有/console/HTTP错误为0。
- 该页用于V2视频10—84秒展示机器技术证据；自动门禁、候选题和固定run仍不升级为专家/课堂效果。
